...
public:
    ...
    //! Constructor
    explicit orb_extractor(const orb_params& orb_params,const aruco::MarkerDetector::Params& maker_params);
...
//! 修改：Compute makers in each image 
    void compute_makers(std::vector<aruco::Marker>& all_makers, const cv::Mat& mask) const;
        //! camera model
...
//修改：参数用于marker的检测
    std::shared_ptr<aruco::MarkerDetector> _MarkerDetector;
