...
keyframe::keyframe(const frame& frm, map_database* map_db, bow_database* bow_db)
    : // meta information
      ...
      markers_(frm.markers_),
      markerpts_(frm.markerpts_),
      // databases
...
void keyframe::add_MarkerObservation(ucoslam::MarkerObservation& mk){
    std::lock_guard<std::mutex> lock(mtx_markers_);
    markers_.push_back(mk);
}

// void keyframe::erase_MarkerObservation(ucoslam::MarkerObservation& mk){
//     std::lock_guard<std::mutex> lock(mtx_markers_);
//     markers_.erase(mk);

// }
int keyframe::get_MarkerIndex(uint32_t id)const  {
    for(size_t i=0;i<markers_.size();i++)
        if (uint32_t(markers_[i].id)==id)return i;
    return -1;

}
std::vector<ucoslam::MarkerObservation> keyframe::get_markers() const{
    std::lock_guard<std::mutex> lock(mtx_markers_);
    return markers_;

}
ucoslam::MarkerObservation keyframe::get_Marker(uint32_t id) const{
...
#pragma warning "try to remove"
 ucoslam::MarkerPosesIPPE keyframe::get_MarkerPoseIPPE(uint32_t id)const
{
    for(size_t i=0;i<markers_.size();i++)
