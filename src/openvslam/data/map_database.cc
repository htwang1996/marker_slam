...
//修改
void map_database::add_Marker(const ucoslam::MarkerObservation* m){
...
void map_database::add_MarkerObservation(uint32_t markerId,uint32_t KeyFrame){
    std::lock_guard<std::mutex> lock(mtx_map_access_);
    assert(keyframes_.count(KeyFrame) == 0);
     assert(map_markers.count(markerId));

     ucoslam::Marker* marker= map_markers.at(markerId);//creates if not yet
     assert(marker->frames.count(KeyFrame)==0);//is not yet added，此maker是否被kf观测到

     //add connection with all frames in this point
    //  for(auto fi:marker.frames)//观测到此maker的kf序列
    //      TheKpGraph.createIncreaseEdge( fi,KeyFrame,4);
     //add the observation of this frame to the marker
     marker->frames.insert(KeyFrame);

}
void map_database::erase_Marker(const ucoslam::MarkerObservation* m){
    std::lock_guard<std::mutex> lock(mtx_map_access_);
    map_markers.erase(m->id);

}
void map_database::erase_MarkerObservation(uint32_t markerId,uint32_t KeyFrame){
    std::lock_guard<std::mutex> lock(mtx_map_access_);
    ucoslam::Marker* marker= map_markers.at(markerId);//creates if not yet
     assert(marker->frames.count(KeyFrame)==0);//is not yet added，此maker是否被kf观测到
    ...
    //      TheKpGraph.createIncreaseEdge( fi,KeyFrame,4);
     //add the observation of this frame to the marker
     marker->frames.erase(KeyFrame);

}

std::vector<ucoslam::Marker*> map_database::get_all_markers() const {
    std::lock_guard<std::mutex> lock(mtx_map_access_);
    std::vector<ucoslam::Marker*> markers;
    markers.reserve(map_markers.size());
    for (const auto id_markers : map_markers) {
        markers.push_back(id_markers.second);
    }
    return markers;
}
