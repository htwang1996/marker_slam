...
#include <ucoslam.h>
#include <aruco.h>
#include "openvslam/optimize/ippe.h"
...
//修改：获取makerid、markerob、markerpose
//    void undistortPoints(vector<cv::Point2f> &points_io,camera::base* camera,vector<cv::Point2f> *out);
    int get_MarkerIndex(uint32_t id)const;
    ucoslam::MarkerObservation get_Marker(uint32_t id) const;
    ucoslam::MarkerPosesIPPE get_MarkerPoseIPPE(uint32_t id)const;
    ...
    //修改：aruco::makers
    std::vector<aruco::Marker> markerpts_;
    std::vector<aruco::Marker> markerpts_right_;
    //ucoslam::makers
    std::vector<ucoslam::MarkerObservation> markers_;//当前帧检测到的maker
    std::vector<ucoslam::MarkerObservation> markers_right_;
    ucoslam::Se3Transform pose_f2g ;//frame pose  Convenion: Global -> This Frame  初始帧在当前帧的位姿，当前帧的旋转为E
...
private:
    ...
    //修改：arucomaker convert to ucoslam maker
     ...
     * @param markers_ob
     */
    void maker_convert(std::vector<aruco::Marker>& all_markers,std::vector<ucoslam::MarkerObservation>& markers_ob);
