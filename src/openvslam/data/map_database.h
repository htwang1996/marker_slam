...
//修改
    void add_Marker(const ucoslam::MarkerObservation* m);
    void add_MarkerObservation(uint32_t markerId,uint32_t KeyFrame);
    void erase_Marker(const ucoslam::MarkerObservation* m);
    void erase_MarkerObservation(uint32_t markerId,uint32_t KeyFrame);
    std::vector<ucoslam::Marker*> get_all_markers() const;
    /**
...
private:
    ...
    ucoslam::SafeMap<uint32_t,ucoslam::Marker*> map_markers;
