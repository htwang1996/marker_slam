...
public:
    ...
    void undistort_markerPoints(std::vector<cv::Point2f> &points_io,std::vector<cv::Point2f> *out)  const override final{
        return ;
    }
    cv::Mat get_cameramatrix() const override final{
        return cv::Mat::eye(4,4,CV_8UC1);
    }
    cv::Mat get_dist_params()const override final{
        	cv::Mat array = (cv::Mat_<double>(4, 1) << 0, 0, 0, 0);
            return array;
    }
