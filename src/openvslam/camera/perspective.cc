...
void perspective::undistort_markerPoints(std::vector<cv::Point2f> &points_io,std::vector<cv::Point2f> *out) const {
    std::vector<cv::Point2f> pout;
    if (out==nullptr) out=&pout;

    assert(cv_cam_matrix_.type()==CV_32F);
    cv::undistortPoints (points_io,*out,cv_cam_matrix_, cv_dist_params_);//results are here normalized. i.e., in range [-1,1]

    float fx=cv_cam_matrix_.at<float> ( 0,0 );
    float fy=cv_cam_matrix_.at<float> ( 1,1 );
    float cx=cv_cam_matrix_.at<float> ( 0,2 );
    float cy=cv_cam_matrix_.at<float> ( 1,2 );
