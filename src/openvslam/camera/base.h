...
//修改：Undistort marker_point
    virtual void undistort_markerPoints(std::vector<cv::Point2f> &points_io,std::vector<cv::Point2f> *out)  const = 0;
    virtual cv::Mat get_cameramatrix() const = 0;
    virtual cv::Mat get_dist_params() const = 0;
