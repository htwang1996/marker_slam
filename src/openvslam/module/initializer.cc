...
//执行完上面的函数后回直接直接退出，然后读取下一帧图像，进入初始化阶段
...
//匹配过程：在匹配完成后，会将匹配到的点的prev_matched_coords_改为当前帧的特征点座标。
//如果匹配到的点数小于min_num_triangulated_（这里是50），则直接重置初始化，会把下一帧当作初始化的参考帧
...
/*创建单目地图
    在init_matches_中，将所有匹配点对中没有三角化的位置标记为无效
    以初始帧为原点，设置当前帧的位姿
    创建初始帧和当前帧的关键帧
    计算帧描述子对应的bow向量
    将关键帧添加入map_db_中
...
bool initializer::create_map_for_monocular(data::frame& curr_frm) {
    ...
    for (unsigned int init_idx = 0; init_idx < init_matches_.size(); init_idx++) {
        ...
        //一个lm可能被很多帧看到，每个帧中由于拍摄的时间、空间、光照条件的原因导致相同的特征点的描述子会稍微不同，
        //通过计算找到一个与其他描述子距离相近的描述子作为最终lm的描述子。
        lm->compute_descriptor();
        // update the geometry 将关键帧中的lm（世界坐标系）转为关键帧坐标系下，对深度排序后获取中间深度值。
