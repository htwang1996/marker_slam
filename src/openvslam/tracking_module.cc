...
tracking_module::tracking_module(const std::shared_ptr<config>& cfg, system* system, data::map_database* map_db,
    ...
    extractor_left_ = new feature::orb_extractor(cfg_->orb_params_,cfg_->marker_params_);
    if (camera_->setup_type_ == camera::setup_type_t::Monocular) {
        ini_extractor_left_ = new feature::orb_extractor(cfg_->orb_params_,cfg_->marker_params_);
        ini_extractor_left_->set_max_num_keypoints(ini_extractor_left_->get_max_num_keypoints() * 2);
    }
    if (camera_->setup_type_ == camera::setup_type_t::Stereo) {
        extractor_right_ = new feature::orb_extractor(cfg_->orb_params_,cfg_->marker_params_);
    }
}
...
void tracking_module::track() {
    ...
    if (tracking_state_ == tracker_state_t::Initializing) {
        ...
        // pass all of the keyframes to the mapping module   将所有的关键帧传入mapper_模块
...
bool tracking_module::track_current_frame() {
    ...
    if (tracking_state_ == tracker_state_t::Tracking) {
        ...
        */
        ...
        // Lost mode
        // try to relocalize  跟踪失败后会调用该函数进行重定位。
...
首先会统计出当前帧可以被观测到的lm，会调用lm->increase_num_observable()，位姿优化后统计inlier的lm，
调用lm->increase_num_observed()，因此num_observed_记录的是跟踪过程中真正有效的lm，
num_observable_记录的是局部地图中可以被当前帧观测到的lm，
如果num_observed_ / num_observable_的值很小的话，说明该lm对位姿评估没有太大意义，可以从局部地图buffer和数据库中清除掉。
*/
