...
//     const auto marker_params = aruco::MarkerDetector::Params();
//     auto extractor = feature::orb_extractor(params,marker_params);
...
//     std::vector<aruco::Marker> markerpts;
//     cv::Mat desc;
//     extractor.extract(img, mask, keypts,markerpts,desc);
...
TEST(orb_extractor, extract_without_mask_2) {
    ...
    // image
    const auto img = cv::imread(std::string(TEST_DATA_DIR) + "./8307.jpg", cv::IMREAD_GRAYSCALE);
    // mask (無効)
    const auto mask = cv::Mat();
...
TEST(orb_extractor, extract_without_mask_3) {
    ...
    // image
    const auto img = cv::imread(std::string(TEST_DATA_DIR) + "./8712.jpg", cv::IMREAD_GRAYSCALE);
    // mask (無効)
    const auto mask = cv::Mat();
